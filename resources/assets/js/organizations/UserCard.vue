<template lang="pug">
    li.list-group-item.cursor-drag
        .d-flex.flex-column
            div.d-flex
                .mr-auto {{ user.name }}
            ...
            div(v-for="calling of user.callings", :key="calling.id")
                ...
                //- small.text-danger(v-if="has_calling_to_release && user.calling") {{ user.calling.name }} (Desobrigação)
                //- small.text-success(v-if="has_new_calling && user.new_calling") {{ user.new_calling.name }} (Novo)
</template>
